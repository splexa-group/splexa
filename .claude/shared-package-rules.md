...
Usage:
- **Server** (`cases-schema.ts`): `import { createCaseSchema } from '@splexa-group/shared/schemas'` → passed to Fastify route `schema: { body: createCaseSchema }`
- **Frontend** (React form): `import { createCaseSchema } from '@splexa-group/shared/schemas'` → used with a form library for field-level validation
...
import { CASE_STATUS, USER_ROLES } from '@splexa-group/shared/constants';
...
│   │   ├── user.ts
│   │   ├── organization.ts
│   │   ├── misc.ts              # Generic envelope types (PaginatedResult, WithWarnings)
│   │   └── index.ts
│   ├── utils/
│   │   ├── date.ts              # formatIndianDate, getRelativeDateLabel
│   │   ├── misc.ts              # maskEmail, etc. — pure functions only
│   │   └── index.ts
│   └── (schemas/, constants.ts — add when needed by both apps)
...
Domain types live under `models/` (not `types/`) — one file per domain entity, plus an `index.ts` barrel per folder. No root `src/index.ts` — see Conditional Exports below.
...
```json
{
  "exports": {
    ...
    "./models": { "types": "./src/models/index.ts", "default": "./dist/models/index.js" }
  }
}
```
...
**No `zod` in shared** — do not add `zod` as a dependency of `packages/shared`. Zod schemas for form inputs (create/update) live in the server module's `schema.ts`, not in shared. The frontend uses its own validation (TBD).
...
1. Create the type file in the correct folder (`models/`, `schemas/`, `utils/`)
2. Export the type from that file
3. Re-export it from the folder's `index.ts`
4. Add a subpath entry point in `tsup.config.ts` if it's a new top-level folder (skip if the folder already has one, e.g. `models/`)
5. Use `import type { MyType } from '@splexa-group/shared/models'` in the consuming apps — there is no root barrel

```ts
// 1. packages/shared/src/models/reminder.ts
export type ReminderType = '3DAY' | '1DAY' | 'MORNING';
export interface Reminder { id: string; caseId: string; type: ReminderType; ... }

// 2. packages/shared/src/models/index.ts
export * from './reminder';

// 3. In apps/server or apps/web
import type { Reminder, ReminderType } from '@splexa-group/shared/models';
```
